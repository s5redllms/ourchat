...
if ($StartServer) {
    ...
    if (Test-Path "app.py") {
        ...
        Write-Host "ERROR: app.py not found. Cannot start server." -ForegroundColor Red
        exit 1
    }
}
...
try {
    ...
    if ($testExitCode -eq 0) {
        ...
        Write-Host "WARNING: Tests identified issues that need attention before production." -ForegroundColor Red
    }
