...
class User(db.Model):
    ...
    user_code = db.Column(db.String(6), unique=True, nullable=False)  # 6-digit unique code
...
class Contact(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # Owner of the contact list
    contact_user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)  # The actual user being contacted
...
class Message(db.Model):
    ...
    message_type = db.Column(db.String(20), default='text', nullable=False)  # text, image, video, gif, file
